...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      setUserId(user_id);
      try {
        const response = await fetch(`http://localhost:6050/find-users`);
        if (!response.ok) {
  ...
  const handleJoinPress = async (selectedUserId) => {
    try {
      const response = await fetch("http://localhost:6050/chat/create", {
        method: "POST",
        headers: {
  ...
  const fetchUserChats = async (userId) => {
    try {
      const response = await fetch(
        `http://localhost:6050/findUserchat/chats/${userId}`
      );
  ...
  const fetchUserName = async (userId) => {
    try {
      const response = await fetch(
        `http://localhost:6050/find/byUserId/${userId}`
      );
