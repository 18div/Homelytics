...
const ChatScreen = () => {
  ...
  const fetchChatMessages = async (chatId) => {
    try {
      const response = await fetch(`http://localhost:6050/chat/get/messages/${chatId}`);
  ...
  const fetchSenderName = async (senderId) => {
    try {
      const response = await fetch(`http://localhost:6050/find/byUserId/${senderId}`);
  ...
  const handleSend = async () => {
    try {
      const response = await fetch(`http://localhost:6050/chat/create/messages`, {
        method: "POST",
        headers: {
