...
function PlacePage() {
  ...
  useEffect(() => {
    const fetchPlace = async () => {
      try {
        const response = await axios.get(`http://localhost:6050/place/${id}`);
  ...
  const handleBook = async () => {
    ...
    try {
      const userBookings = await axios.get(`http://localhost:6050/user/bookings?userId=${userId}`);
      ...
      if (overlappingBooking) {
        ...
        const response = await axios.post("http://localhost:6050/book", bookingData);
